...
export const useMultimedia = () => {
    ...
    const [broadcasts, _setBroadcasts] = useState<Broadcast[]>(() => {
    ...
    const [channelSettings, _setChannelSettings] = useState(() => {
        if (typeof window !== 'undefined') {
        ...
        try {
            ...
            if (broadcastsRes.ok) {
                ...
                if (Array.isArray(broadcastsData) && broadcastsData.length > 0) {
                    ...
                    localStorage.setItem('broadcasts_data_v10', JSON.stringify(mappedBroadcasts));
            ...
            if (settingsRes.ok) {
                ...
                if (Array.isArray(settingsData) && settingsData.length > 0) {
                    ...
                    localStorage.setItem('multimedia_settings_v10', JSON.stringify(mappedSettings));
    ...
    const setBroadcasts = useCallback((val: Broadcast[] | ((prev: Broadcast[]) => Broadcast[])) => {
        _setBroadcasts(prev => {
            ...
            localStorage.setItem('broadcasts_data_v10', JSON.stringify(next));
    ...
    const setChannelSettings = useCallback((val: any | ((prev: any) => any)) => {
        _setChannelSettings(prev => {
            const next = typeof val === 'function' ? val(prev) : val;
            localStorage.setItem('multimedia_settings_v10', JSON.stringify(next));
